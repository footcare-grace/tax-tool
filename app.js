...
/* ===== 状態管理 ===== */
const LS_KEY="keihi-data-v1", LS_RULES="keihi-rules-v1", LS_CATS="keihi-cats-v1";
let rows, rules, cats;
...
function renderSummary(){
  ...
  $("#stat-cards").innerHTML=`
    <div class="stat"><div class="lbl">登録件数</div><div class="val">${rows.length}件</div></div>
    <div class="stat"><div class="lbl">支払総額</div><div class="val">${yen(totalA)}</div></div>
...
$("#btn-pdf").addEventListener("click",()=>{
  ...
  let html=`<h1>経費明細書（事業所得・雑所得 申告用）</h1>
  <p>対象期間：${dates[0]||"—"} 〜 ${dates[dates.length-1]||"—"}　／　作成日：${new Date().toLocaleDateString("ja-JP")}　／　件数：${rows.length}件</p>
  <h2>カテゴリ別集計</h2>
...
$("#btn-wipe").addEventListener("click",()=>{
  if(confirm("全データを削除します。よろしいですか？")&&confirm("本当に削除しますか？（この操作は取り消せません）")){
    rows=[]; save(); render();
  }
});

function render(){ renderList(); renderSummary(); renderRules(); }
